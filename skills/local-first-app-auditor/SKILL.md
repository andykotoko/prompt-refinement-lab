...
- 当用户只是要修一个孤立的前端小问题，且不涉及页面编排、存储边界、原生壳体或交付问题时，不必展开完整审计
- 当项目并非 Local-First / Hybrid / Desktop 风格，而是普通的纯 SaaS Web 应用时，应优先使用更通用的项目审计 skill
- 当用户只是单纯想找视觉灵感或做风格脑暴时，这个 skill 不是最佳入口

## Quick Reference

| 当前需求 | 更适合这个 skill 的输出 |
|---|---|
| 页面越来越大、职责越来越乱 | 架构与页面编排审计 |
| 存储适配器越来越复杂 | 数据闭环与 adapter 边界审计 |
| Tauri / Electron 层开始侵蚀业务逻辑 | Native Layer 边界审计 |
| 文档说支持多平台，但心里没底 | 发布与交付可信度审计 |
...
在结束一次 Local-First 审计前，优先自检：

- [ ] 是否确认项目确实属于 Local-First / Hybrid / Desktop 语境
- [ ] 是否区分了“视觉表皮问题”和“视觉系统可维护性问题”
- [ ] 是否给出了真实文件或模块作为证据
- [ ] 是否把平台承诺与实际 workflow / release 证据做了对照
- [ ] 是否优先建议收束边界，而不是直接建议重写
