...
- 当用户只需要一个很小的代码改动，且不存在治理、契约、规划或文档漂移问题时，不必强行展开完整流程
...
| 当前需求 | 更适合的模式 | 常见输出 |
|---|---|---|
| 新建项目骨架 | **Bootstrap** | AGENTS.md / CONSTITUTION.md / MANUAL.md |
...
- 在结束一次 Project Forge 任务前，优先自检：

- [ ] 是否已基于真实仓库结构，而不是模板化想象
- [ ] 是否区分了“应立即生成”与“仅建议后续补充”的文档
- [ ] 如果输出了 PRD / ADR，是否避免写具体文件路径和易过时的实现细节
- [ ] 如果提出 contract 建议，是否说明边界输入、输出与错误形状
- [ ] 如果判断存在漂移，是否给出具体证据路径而不是笼统印象
- [ ] 如果任务其实很小，是否避免把流程扩展得过重
