...
- `CONSTITUTION.md`、`AGENTS.md`、`CLAUDE.md` 这类 agent / governance 文件通常应以**单一规范语言**维护，避免多语言版本产生规则漂移。
- 面向用户的文档（如 `README*.md`、帮助页、安装指南）才是优先多语言化的对象。
...
- [ ] 是否把 locale 限定在展示层，而不是 schema / contract 层
- [ ] 是否避免让 enum / key / ID / 中间对象命名受 locale 影响
- [ ] 是否明确说明了翻译文件组织方式与命名规范
- [ ] 是否指出了哪些地方应该本地化、哪些地方不应该本地化
- [ ] 是否区分了“治理文件通常单语”与“用户文档应多语”的职责边界
- [ ] 如果在做改造，是否给出渐进迁移顺序，而不是一次性大改
